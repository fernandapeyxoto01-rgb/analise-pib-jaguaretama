...
with st.sidebar:
    ...
    with st.container(height=420):
        for msg in st.session_state.chat_messages[-20:]:
            ...
            st.markdown(
                f'<div style="background:{cor};border-radius:8px;padding:10px 12px;'
                f'margin-bottom:8px;font-size:0.95rem;line-height:1.5;">'
...
if send_clicked and user_question.strip():
    ...
    try:
        _chave = os.getenv("OPENAI_API_KEY")
        if not _chave:
...
if st.button("Exportar Excel (período filtrado)"):
    ...
    st.success(f"✅ Arquivo exportado: {nome}")
